...
CELLS = [
    ...
    nbf.v4.new_code_cell(
        "delivery = pd.Timestamp('2026-05-01').date()\n"
        "ts_filter = lambda df: df[df['issue_date'] == str(delivery)]\n"
        "tso_day = ts_filter(tso_df)\n"
        "plain_day = ts_filter(plain)\n"
        "weather_day = ts_filter(weather)\n"
        "\n"
        "if not tso_day.empty:\n"
        "    fig = go.Figure()\n"
        "    fig.add_trace(go.Scatter(x=tso_day['target_ts'], y=tso_day['y_true'],\n"
        "                              name='actual', line=dict(color='black', width=2.5)))\n"
        "    fig.add_trace(go.Scatter(x=tso_day['target_ts'], y=tso_day['y_tso'],\n"
        "                              name='TSO baseline', line=dict(color='#2E86AB', dash='dot')))\n"
        "    fig.add_trace(go.Scatter(x=plain_day['target_ts'], y=plain_day['y_model'],\n"
        "                              name='LSTM plain', line=dict(color='#7B7B7B')))\n"
        "    fig.add_trace(go.Scatter(x=weather_day['target_ts'], y=weather_day['y_model'],\n"
        "                              name='LSTM weather', line=dict(color='#C73E1D', width=2)))\n"
        "    fig.update_layout(\n"
        "        title=f'{delivery} — actual vs three forecasts',\n"
        "        xaxis_title='Time (UTC)', yaxis_title='Grid load (MW)',\n"
        "        height=420, template='plotly_white', hovermode='x unified',\n"
        "    )\n"
        "    fig.show()\n"
        "    mae_tso     = (tso_day['y_true'] - tso_day['y_tso']).abs().mean()\n"
        "    mae_plain   = (plain_day['y_true'] - plain_day['y_model']).abs().mean()\n"
        "    mae_weather = (weather_day['y_true'] - weather_day['y_model']).abs().mean()\n"
        ...
        "else:\n"
        "    print(f'No backtest entry for {delivery} (step_days=7 may have skipped it)')\n"
    ),

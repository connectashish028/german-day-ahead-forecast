"""Train the seq2seq LSTM with Bahdanau attention.

Same splits, scaler, and training config as `train_lstm_plain.py` so the
comparison is apples-to-apples — only the model architecture differs.

Saves to model_checkpoints/lstm_attention_v1/.
"""
...
from loadforecast.models.lstm_attention import (
    build_lstm_attention_train,
    compile_lstm_attention,
)

PARQUET = "smard_merged_15min.parquet"
OUT_DIR = Path("model_checkpoints/lstm_attention_v1")
...
def main() -> None:
    ...
    print(f"\nBuilding windows: {len(train_dates)} train, {len(val_dates)} val")
    ...
    model = compile_lstm_attention(build_lstm_attention_train(hidden=64), lr=1e-3)

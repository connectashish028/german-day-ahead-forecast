...
MODEL_DIR = Path("model_checkpoints/price_quantile_v1")
...
def _predict_quantile(model, scaler, df, issue):
...
def main() -> None:
    ...
    print(f"  model: trained {meta['train_n']} days, val P50 MAE = "
          f"{meta['val_p50_mae_eur_mwh']:.2f} €/MWh")

    holdout = list(_drange(date(2026, 3, 1), date(2026, 4, 30)))
    print(f"\nBacktesting {len(holdout)} holdout days...")

    rows = []
    for d in holdout:
        issue = issue_time_for(d)
        result = _predict_quantile(model, scaler, df, issue)
        if result is None:
            continue
    ...
    print()
    print("=" * 60)
    print(f"Holdout: {n_days} days, {len(bt):,} 15-min slots")
    print("=" * 60)
    print(f"\nP50 point MAE (€/MWh):")
    ...
    print(f"\nDaily price-spread MAE (€/MWh) — what battery P&L depends on:")
    ...
    print(f"  Mean width:         {width:.1f} €/MWh")

    OUT_CSV.parent.mkdir(parents=True, exist_ok=True)
    bt.to_csv(OUT_CSV, index=False)
    print(f"\nWrote {OUT_CSV}  ({OUT_CSV.stat().st_size // 1024} KB)")

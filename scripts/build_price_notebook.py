...
CELLS = [
    ...
    nbf.v4.new_markdown_cell(
        "## 5. Training\n\n"
        "*Pending.* Next session: train `model_checkpoints/price_quantile_v1/` on "
        "the train/val splits above using the same `lstm_quantile.py` architecture. "
        "~5 min on CPU."
    ),

    nbf.v4.new_markdown_cell(
        "## 6. Backtest vs naive baselines\n\n"
        "*Pending.* Compute holdout MAE / MAPE / coverage vs:\n\n"
        "- Naive: yesterday-same-quarter-hour\n"
        "- Naive: last-week-same-quarter-hour (captures weekly seasonality)\n\n"
        "Senior-DS-aware metrics: focus on **price-spread accuracy** "
        "(max - min within the day), not just point error — that's what battery "
        "P&L actually depends on."
    ),

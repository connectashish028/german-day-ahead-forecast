...
CELLS = [
    ...
    nbf.v4.new_markdown_cell(
        ...
        "Could add later.\n"
        "- **TSO renewable forecast** — we removed that source last week because of "
        "manual-CSV friction in CI. Re-add via downloadcenter API if it helps."
    ),
    ...
    # ----- 6. Backtest -----
    nbf.v4.new_markdown_cell(
        "## 6. Backtest on the holdout (Mar–Apr 2026)\n\n"
        ...
        "The headline numbers, plus a metric a trading DS would insist on: **daily "
        "price-spread MAE**. Battery dispatch makes money from `max(price) − "
        "min(price)` *within a day*, not from average accuracy. A model with low "
        "point MAE but bad spread accuracy is **worse for trading** than a noisy "
        "model that captures the spread."
    ),
    ...
    nbf.v4.new_markdown_cell(
        "### What this means\n\n"
        "**The headline +18 % point-MAE improvement is real but misleading.** "
        "A senior DS reading this would notice the daily-spread MAE has gone the "
        "*other way* — model is **65 % worse** than the yesterday-naive at "
        "predicting the day's price spread.\n\n"
        "Why? The P50 is the *median* by construction; pinball-loss minimisation "
        "drags it toward the centre of the distribution. Daily extremes get "
        "compressed. So `max(p50) − min(p50)` consistently under-estimates "
        "`max(actual) − min(actual)`.\n\n"
        "**The implication for battery dispatch (Section 8):** don't use P50 as "
        "the input to the dispatch optimisation. Use **P10 prices** to find cheap "
        "charging slots and **P90 prices** to find expensive discharging slots. "
        "The wide bands exist *because* the model is uncertain at the extremes — "
        "we can use that uncertainty productively instead of throwing it away."
    ),

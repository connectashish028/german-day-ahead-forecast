...
CELLS = [
    ...
    nbf.v4.new_markdown_cell(
        "## 5. Training\n\n"
        "Reused `lstm_quantile.py` verbatim — 64-unit encoder + 64-unit "
        "decoder + 3 quantile heads. The current shipped model is **v4**, "
        "trained on 2022-01-08 → 2025-12-31 with sample weighting (3× "
        "federal holidays + Sundays, 0.5× 2022–2023) and 30 % feature-"
        "dropout on `tso_vre_fc`. Saved to "
        "`model_checkpoints/price_quantile_v4/`. Section 9 covers the "
        "feature-dropout (M8); section 10 covers the sample weighting + "
        "engineered features (M9).\n\n"
    ...
    nbf.v4.new_code_cell(
        ...
        "from pathlib import Path\n"
        "\n"
        "meta = json.loads(Path('model_checkpoints/price_quantile_v4/meta.json').read_text())\n"
        "rows = [\n"
    ...
    nbf.v4.new_markdown_cell(
        ...
        "architecture, same loss, same training recipe.\n\n"
        "**v3 — graceful degradation.** v2 hard-failed when SMARD's day-ahead "
        "VRE forecast hadn't published yet (publishes ~D-1 12:30 Berlin, "
        "sometimes later). v3 adds a `tso_vre_fc_present` flag and trains "
        "with 30 % feature-dropout on `tso_vre_fc` so the model learns to "
        "fall back to weather + load + calendar when the operator hasn't "
        "published. The cost is ~+0.7 €/MWh validation MAE in full-feature "
        "mode; the gain is the model issues a forecast all day, not just "
        "after 12:30. Production pattern. Detailed in section 9.\n\n"
        "**v4 — tail-aware retrain.** v3 still missed the deepest negative-"
        "price events (May 1, 2026 hit −500 €/MWh; v3 predicted near zero). "
        "v4 adds two engineered decoder features (`vre_to_load_ratio`, "
        "`vre_percentile`), re-includes 2022–2023 with 0.5× sample weight to "
        "recover negative-price examples, and 3× upweights federal holidays "
        "+ Sundays. Average MAE essentially unchanged (23.7 vs 23.8 €/MWh); "
        "worst-10 % days improve dramatically (60 % better than naive vs "
        "roughly tied before). Detailed in section 10.\n\n"
        "**The numbers below are v4** (current shipped model)."
    ),
    ...
    nbf.v4.new_markdown_cell(
        ...
        "The deployed Streamlit dashboard renders tomorrow's P10/P50/P90 "
        "price forecast alongside the load forecast — same parquet, same "
        "model checkpoint as this notebook. When SMARD's VRE day-ahead "
        "forecast hasn't published yet, the dashboard shows a "
        "`DEGRADED MODE` badge and the model runs on weather + load only "
        "(~+38 % MAE expected). See the **PRICE** view in the live "
        "dashboard for any past delivery day."
    ),
    ...
    # ----- 9. Graceful degradation (M8) -----
    nbf.v4.new_markdown_cell(
        "## 9. Graceful degradation when SMARD's VRE forecast is late\n\n"
        "**The pain point.** SMARD's `fc_gen__photovoltaics_and_wind` (the "
        "dominant price driver) publishes at D-1 ~12:30 Berlin, sometimes "
        "as late as 13:00 if a TSO's pipeline is slow. A naive "
        "implementation hard-fails on missing decoder input: nothing "
        "rendered until SMARD publishes, every day. A trader running this "
        "model at 09:00 to shape bids has nothing to look at.\n\n"
        "**The fix.** Two changes that mirror what real desks do:\n\n"
        "1. Decoder feeds `tso_vre_fc` *and* a binary `tso_vre_fc_present` "
        "flag. The windowing path imputes NaN → 0 on the VRE column when "
        "SMARD hasn't published.\n"
        "2. Training augments 30 % of windows by zeroing `tso_vre_fc` and "
        "the present flag. The model sees both regimes and learns to "
        "lean on weather + calendar in the missing case.\n\n"
        "**Validation cost.** Full-feature MAE: 13.4 €/MWh. VRE-masked "
        "MAE: 18.6 €/MWh (+38 %). The model is meaningfully worse "
        "without the dominant feature — but it produces a forecast at "
        "all, which is what trading desks need from 09:00 onwards.\n\n"
        "**Holdout (61 days, both modes).** Even masked, the model still "
        "beats naive yesterday by 19 % point MAE; full-feature mode wins "
        "by 36 %. Battery dispatch P&L: 86.6 % of perfect-foresight "
        "masked vs 94.9 % full — both meaningfully above the naive "
        "baseline's 81 %.\n\n"
        "The dashboard surfaces a `DEGRADED MODE` badge whenever the "
        "model is running without `fc_gen` so the user knows what "
        "they're looking at."
    ),

    nbf.v4.new_code_cell(
        "# Compare full-feature vs VRE-masked holdout side by side.\n"
        "import pandas as pd\n"
        "from pathlib import Path\n"
        "\n"
        "rows = []\n"
        "for label, path in [\n"
        "    ('Full features',  'backtest_results/price_quantile_holdout.csv'),\n"
        "    ('VRE masked',     'backtest_results/price_quantile_holdout_vre_masked.csv'),\n"
        "]:\n"
        "    if not Path(path).exists():\n"
        "        continue\n"
        "    bt = pd.read_csv(path, parse_dates=['target_ts'])\n"
        "    mae_model = (bt['y_true'] - bt['p50']).abs().mean()\n"
        "    mae_naive = (bt['y_true'] - bt['naive_1d']).abs().mean()\n"
        "    rows.append({\n"
        "        'Mode': label,\n"
        "        'P50 MAE (€/MWh)':       f'{mae_model:.2f}',\n"
        "        'Naive MAE (€/MWh)':     f'{mae_naive:.2f}',\n"
        "        'Skill vs naive':         f'{(1-mae_model/mae_naive)*100:+.1f} %',\n"
        "    })\n"
        "pd.DataFrame(rows)\n"
    ),

    # ----- 10. Tail-aware retrain (M9) -----
    nbf.v4.new_markdown_cell(
        "## 10. Tail-aware retrain for extreme regimes\n\n"
        "**The failure case.** May 1, 2026 — federal holiday + 54 GW PV "
        "record — saw prices slammed against the negative-subsidy floor "
        "at −500 €/MWh for four hours. The v3 model predicted near zero. "
        "The naive baseline did better because April 30 was already "
        "partly negative. Same pattern on April 26 (−480 €/MWh).\n\n"
        "**Three root causes.**\n\n"
        "1. **Median collapse on a rare regime.** Pinball loss pulls P50 "
        "toward the *conditional* median given the features. There are "
        "maybe 5–10 holiday-with-record-PV days in the training set; the "
        "conditional median across those is much shallower than −500.\n"
        "2. **No mechanistic feature.** The model has to learn `vre / load` "
        "from two raw scaled inputs.\n"
        "3. **Training window cut at 2024.** Done to dodge the 2022 €500 "
        "spike outliers, which also dropped most of the deepest "
        "negative-price events.\n\n"
        "**What v4 changes.**\n\n"
        "| Change | Effort | Why |\n"
        "|---|---|---|\n"
        "| Add `vre_to_load_ratio` + `vre_percentile` decoder features | low | Mechanistic price driver, no longer hidden inside the LSTM |\n"
        "| Re-include 2022–2023 with 0.5× sample weight | low | Doubles holiday/extreme examples without spike domination |\n"
        "| 3× sample weight on federal holidays + Sundays | low | The exact regime that broke v3 |\n\n"
        "**Results on a 65-day extended holdout (Mar 1 → May 5, 2026):**\n\n"
        "| Slice | v4 MAE | Naive MAE | Improvement |\n"
        "|---|---|---|---|\n"
        "| All days | 25.0 | 38.3 | +34.7 % |\n"
        "| Worst 10 % (by naive) | 32.7 | 82.0 | **+60.1 %** |\n"
        "| Holidays / weekends (n=22) | 27.7 | 42.5 | +34.8 % |\n"
        "| Negative-price days (n=6) | 45.7 | 56.3 | +18.8 % |\n"
        "| May 1 specifically | 81.8 | 77.9 | **−5.0 %** |\n\n"
        "**The honest May 1 finding.** v4 made the P50 commit deeper "
        "into the negatives (`p50_min` improved from ~0 to −38 €/MWh) "
        "and now wins on 4 of 6 negative-price days plus the shoulder "
        "extremes (May 2, May 4, March 4). But the absolute deepest tail "
        "— actuals at −500 — remains structurally unreachable by a "
        "continuous-output LSTM trained on pinball loss: the conditional "
        "median given features stays around the conditional median, no "
        "matter how many extreme examples we add. To predict −500 we'd "
        "need either a regime classifier or a domain-rule clipper.\n\n"
        "**Battery P&L unchanged at 95 % of perfect-foresight, +€57 k vs "
        "naive over 61 days.** Dispatch is a ranking problem, not a "
        "level problem (the surprise from Section 8); v4's tail "
        "improvements help on bad-forecast days that v3 didn't get "
        "right, but the average dispatch outcome on routine days is the "
        "same."
    ),
]
